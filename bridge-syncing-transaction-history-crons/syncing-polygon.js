...
var MATIC_CHAIN_ID = 137;	// chain ID  is for the Polygon MATIC Mainnet
var DTH_CHAIN_ID = 24;	// chain ID 24 is for the Dithereum Mainnet
var DEFAULT_COIN = "MATIC";
var FROM_TOKEN = "MATIC";
var TO_TOKEN = "MATIC";
var CONTRACT_ADDR = '0x75287F2e834c390d9D5A83e9F4b60Ca6B1930c6b';	//POLYGON MATIC
...
var httpprovider = new Web3(new Web3.providers.HttpProvider("https://polygon-rpc.com", options));
...
async function execute(){
		...
		try{
			...
			if(lastBlockData[0]){
				...
				console.log(lastBlockData[0].polygon);
				var lastBlockDatabase = lastBlockData[0].polygon;
		 				...
		 				for(var i=0;i<eventlen; i++){		 						 										
							...
							if(parseInt(_amount)){							
								try{
									...
									var insert_query = "INSERT INTO bridge_transactions (`userWallet`,`orderID`,`fromChain`,`fromCurrency`,`fromTxnHash`,`fromAmount`,`toChain`,`toCurrency`,`toTxnHash`,`toAmount`) VALUES ('"+_userWallet+"',"+_orderid+","+MATIC_CHAIN_ID+",'"+FROM_TOKEN+"','"+_txnHash+"',"+_amount+","+DTH_CHAIN_ID+",'"+TO_TOKEN+"','"+_toTxnHash+"','"+_toAmount+"')";		
									console.log(">>> Inserting record, orderid, transactionHash >>>",_orderid, _txnHash);
									await db_query(insert_query).catch(console.log);
		 				...
		 				for(var i=0;i<eventlen; i++){		 						 										
							...
							if(parseInt(_amount)){							
								try{
									...
									var select_from_txn_query = "SELECT * FROM bridge_transactions where toChain='"+MATIC_CHAIN_ID+"' && orderID='"+_orderid+"' && status='Pending'";	
									...
									if(fromTxnData[0]){	
										var update_query = "UPDATE `bridge_transactions` SET `toTxnHash`='"+_txnHash+"',`toAmount`='"+_amount+"',`status`='Completed' WHERE `toChain`='"+MATIC_CHAIN_ID+"' && `orderID`='"+_orderid+"'";		
										...
										console.log("Unprocessed Transaction: chain, Hash: "+MATIC_CHAIN_ID+" - "+_txnHash);
									}
...
async function lastBlockWorked(_lastBlocknumber){	
  	...
  	var sql = "UPDATE lastblock SET polygon="+_lastBlocknumber+" LIMIT 1";
  	console.log("<<< SQL >>>",sql);
