    ...
    await getAvailableAdminWallet_bridge(bridgeweb3, _chainid).then(() => {
        ...
        if (typeof(JSON.parse(_envobj)['walletid']) == "undefined") {
            ...
            (async () => {
                ...
                requiredGas = await company_bridgeinstance.methods.tokenOut(HT_TOKEN_ADDRESS, _toWallet, _amt, orderid, _chainid).estimateGas({
...
var job = new CronJob('0 */2 * * * *', function() {
    console.log("--------------------------------------------------");
    console.log('Cron running, every 2 mins [HT-single script]');
    console.log("--------------------------------------------------");
    // DONE Changes
