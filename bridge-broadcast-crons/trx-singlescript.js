    ...
    try {
        ...
        if (_adminwallet[0]) {
            ...
            await bridgeweb3.eth.getTransactionCount(_xobj['walletid'], "pending").then((z) => {
                console.log(">>>>>>_xobj['walletid'], z >>>>>", _xobj['walletid'], z);
...
async function company_bridge_send_method_coinin(_toWallet, _amt, orderid, _chainid = 34) {
    ...
    const gasPrice = await bridgeweb3.eth.getGasPrice();
    ...
    await getAvailableAdminWallet_bridge(bridgeweb3, _chainid).then(() => {
        ...
        if (typeof(JSON.parse(_envobj)['walletid']) == "undefined") {
            ...
            var requiredGas = 0;
            // This is test amount
            _amt = _amt / 10000; // JUST FOR TESTING SMALL AMOUNT AS NOT ENOUGH COINS/TOKENS TO TEST	    		  			    
            (async () => {
            ...
            var nonc;
            if (x3['lastnonce'] == null) {
                nonc = 0;
            } else if (parseInt(x3['lastnonce']) == 0) {
                nonc = 0;
            } else {
                nonc = x3['lastnonce'];
            }
            ...
            console.log("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
            console.log("nonc, process.env.LAST_NONC >>> ", nonc, process.env.LAST_NONC);
            console.log("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");

            // HARD SET if nonc, not updated before next transaction
            nonc = (parseInt(process.env.LAST_NONC) > parseInt(nonc)) ? parseInt(process.env.LAST_NONC) : parseInt(nonc);
                ...
                bridgeweb3.eth.accounts.signTransaction(raw_tx, JSON.parse(_envobj)['walletpk'].toString(), function(error, result) {
                    if (!error) {
                        try {
                            ...
                            process.env.LAST_NONC = parseInt(nonc) + 1;
                            ...
                            console.log("--------------------------------------------------------");
                            console.log(">>>>>> NONCE >>>>>", nonc + 1);
                            console.log("--------------------------------------------------------");
                            update_nonce(_chainid, x2['walletid'].toString(), parseInt(nonc) + 1);
                        } catch (e) {
                            console.log(e);
                        }
                    }
...
async function getEventData_CoinIn() {
    ...
    // lets pick approx. last 500 trasactions
