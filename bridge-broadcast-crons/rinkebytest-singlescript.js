...
async function bridge_sendmethod(_toWallet, _amt, orderid, _chainid) {
    ...
    var requiredGas = await company_bridgeinstance.methods.tokenOut(ETH_TOKEN_ADDRESS.toString(), _toWallet.toString(), _amt.toString(), orderid.toString(), _chainid.toString()).estimateGas({
...
async function company_bridge_send_method(_tokenaddr, _toWallet, _amt, orderid, _chainid) {
    ...
    var requiredGas = await company_bridgeinstance.methods.tokenOut(pairedwithContract.toString(), _toWallet.toString(), _amt.toString(), orderid.toString(), _chainid.toString()).estimateGas({
        from: process.env.ADMIN_WALLET_BRIDGE.toString()
    }).catch(console.log);
